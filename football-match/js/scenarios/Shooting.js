/**
 * Shooting - 하프라인 오른쪽 20m 지점에서 드리블 후 슈팅
 *
...
const START_X = HALF_LINE_X + 20 * 10;
...
function randomSpeed() {
    return SPEEDS[1 + Math.floor(Math.random() * 4)];
}
...
function createDribblePlan() {
    ...
    const firstX = START_X + Math.min(20 + Math.random() * 20, shootX - START_X - 8);
    const firstY = clamp(CENTER_Y + (Math.random() * 2 - 1) * 35, 240, 440);
    const finalY = clamp(CENTER_Y + (Math.random() * 2 - 1) * 20, 260, 420);

    return [
        { x: firstX, y: firstY, speed: randomSpeed() },
        { x: shootX, y: finalY, speed: randomSpeed() },
    ];
}

function randomAimY() {
    const aim = Math.floor(Math.random() * 3);
    if (aim === 1) return GOAL_TOP_Y - 8 + Math.random() * 15;
    if (aim === 2) return GOAL_BOTTOM_Y - 7 + Math.random() * 15;
    return CENTER_Y;
}

function randomShotHeight() {
    const roll = Math.random();
    if (roll < 0.28) return { targetHeight: 0.06, arcHeight: 0.08 };
    if (roll < 0.78) {
        return {
    ...
    // 골대 위로 넘어가는 슛
    return { targetHeight: 2.65 + Math.random() * 0.35, arcHeight: 0.08 };
}
...
export function run(layer, loop, onComplete = null) {
    ...
    function keepAttackingGoal() {
        if (shootReady || recovering || !currentWaypoint) return;
        pm.setFacingTarget(angleTo(player.x, player.y, GOAL_X, CENTER_Y));
    }
    ...
    function fireShot() {
        ...
        const targetAngle = angleTo(player.x, player.y, GOAL_X, targetY);
